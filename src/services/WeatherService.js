...
export const getLocationsMatchAnAutocomplete = (city) => {
    return new Promise(resolve => setTimeout(() => "Tel Aviv" === city ? resolve(telAvivAutocomplete) : resolve(locationsMatchAnAutocomplete), 2000));
}

export const getCityWeatherForOneDay = (cityKey) => {
    return new Promise(resolve => setTimeout(() => 215854 === cityKey ? resolve(telAvivIsraelWeatehrOneDay) : resolve(moscowRussionWeatehrOneDay), 2000));
}

export const getCityWeatherForLastFiveDay = (cityKey) => {
    return new Promise(resolve => setTimeout(() => 215854 === cityKey ? resolve(telAvivIsraelWeather5Day) : resolve(moscowRussionWeather5Day), 2000));
} 